...
const EquipmentForm: FC<Props> = props => {
  ...
  const [serviceAddressId, setServiceAddressId] = useState<number>(0);
    ...
    if (isEdit) {
      ...
      const getMainEquipmentList = async () => {
        try {
          ...
          params.append('ci', equipment.clientId!.toString());
      ...
      const getServiceAddress = async () => {
        const serviceAddresses = await getServiceAddresses(equipment.clientId!);
      ...
      if (isMain) {
        ...
        setTempClientValue({ id: clientId!, name: clientName!, firstServiceAddress: '', ContactPersons: [] });
        ...
        const { SubEquipments, serviceAddressId, clientId, clientName } = equipment;
        if (!SubEquipments) {
          return;
        }
      ...
      const { serviceAddressId, brand, model, serialNumber, location, address, clientId, clientName } = equipment;
      setTempClientValue({ id: clientId!, name: clientName!, firstServiceAddress: '', ContactPersons: [] });
      setClientName(clientName!);
      setServiceAddressId(serviceAddressId);
      setBrand(brand);
  ...
  const handleChangeClientName = async (value: any) => {
    if (value) {
      ...
      setServiceAddressMaster(serviceAddresses);
      setServiceAddressId(serviceAddresses[0].id);
      setAddress(serviceAddresses[0].name);
  ...
  const validateForm = () => {
    ...
    if (!tempClientValue || tempClientValue.id === 0) {
      setClientNameError('Please select a client name');
      ret = false;
    } else {
      setClientNameError('');
    }
    ...
    if (currentErrorMessage[0].warrantyStartDate !== '') {
      ret = false;
    }

    if (currentErrorMessage[0].warrantyEndDate !== '') {
      ret = false;
    }
    ...
    if (subEquipments.length > 0) {
      subEquipments.forEach((value, index) => {
        ...
        if (currentErrorMessage[index + 1].warrantyStartDate !== '') {
          ret = false;
        }

        if (currentErrorMessage[index + 1].warrantyEndDate !== '') {
          ret = false;
        }
      });
    }
      ...
      {isEdit ? (
        ...
        <>
          {isAddSub ? (
            ...
            <Grid item xs={12}>
              <Card variant='outlined' className={classes.card} style={{ marginBottom: 16 }}>
                <CardHeader
                  title={<Typography variant='subtitle2'> Client Information</Typography>}
                  ...
                  <Grid container spacing={2} alignItems='center'>
                    ...
                    <Grid item xs={12} md={9}>
                      <Autocomplete
                        ...
                        onBlur={() => {
                          const matched = clientOptions.find(c => c.name === clientName);
                          if (!matched) {
                            setClientName('');
