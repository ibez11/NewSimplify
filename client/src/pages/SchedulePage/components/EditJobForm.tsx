...
import { Grid, TextField, Theme, Typography, Button, DialogActions, MenuItem, Chip, Checkbox, Divider, Box } from '@material-ui/core';
...
import { disablePrevDates, getUnique, ucWords } from 'utils';
...
const EditJobForm: FC<Props> = props => {
  ...
  const { jobEvent, employeeMaster, vehicleMaster, publicHolidays, handleClose, handleSnackbar, fetchData } = props;

  const { clientName, contactPerson, contactNumber, serviceAddress } = jobEvent;
  ...
  useEffect(() => {
    ...
    if (employeesSelected) {
      employeesSelected.map((value: any) => {
        return getEmployees.push({ id: value.id, name: value.name });
      });
    }

    if (vehicleSelected) {
      vehicleSelected.map((value: any) => {
    ...
    if (Skills) {
      getSkills = Skills.map(item => item.skill);
    }
  ...
  const handleSetEmployee = (employees: Select[]) => {
    ...
    // setJobStatus(selectedEmployee.length > 0 ? 'ASSIGNED' : 'UNASSIGNED');
    setEditJob({
      ...editJob,
      selectedEmployees: selectedEmployee,
      jobStatus: selectedEmployee.length > 0 ? JobStatus.ASSIGNED : JobStatus.UNASSIGNED
    });
    setEmployeeError('');
  };
  ...
  const handleSubmit = async () => {
    const cancelTokenSource: CancelTokenSource = axios.CancelToken.source();
    const { jobId, startDateTime, endDateTime } = editJob;
  ...
  return (
    <Grid container spacing={2} className={classes.contentGrid} alignItems='center'>
      <Grid item container spacing={2} xs={12} style={{ marginBottom: 16 }}>
        ...
        <Grid item xs={12} sm={4}>
          ...
          <Typography variant='subtitle1'>({serviceAddress})</Typography>
        </Grid>
      </Grid>
      <Grid item xs={12} sm={4}>
      ...
      {cancelConfirmation && (
        <CancelConfirmation
          ...
          handleClose={() => {
            ...
            setEditJob({ ...editJob, jobStatus: jobEvent.jobStatus });
          }}
          setOpenPasswordConfirmation={setOpenPasswordDialog}
          isDeduct={isDeduct}
          setIsDeduct={setIsDeduct}
          invoiceNumber={jobEvent.invoiceNumber || ''}
        />
      )}
      {openPasswordDialog && (
        <PasswordConfirmation
          ...
          url={GET_CANCEL_JOB_URL(Number(jobEvent.jobId))}
          isEdit={true}
