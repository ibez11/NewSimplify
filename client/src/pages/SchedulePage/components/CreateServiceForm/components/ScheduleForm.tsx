...
import { convertTz, disablePrevDates, getNewDate, getNumberWithOrdinal, getUnique, ucWords } from 'utils';
import { grey } from '@material-ui/core/colors';
import { ServiceBody } from 'typings/body/ServiceBody';
import { isValid, addMonths, format, isBefore, getDay, isSameDay, addDays } from 'date-fns';
...
const ScheduleForm: FC<Props> = props => {
  ...
  const [selectedTab] = useState<any>(0);
  const [activeGenerate, setActiveGenerate] = useState<boolean>(false);
  const [schedules, setSchedules] = useState<ScheduleModel[]>([{ ...dummySchedule }]);
          ...
          <Grid container spacing={2}>
            <Grid item xs={12} md={4}>
              <Typography variant='h6'>
                ...
                <FormHelperText>
                  You only can create Ad-hoc Quotation, if you wish to create Service Contract please create it from client page
                </FormHelperText>
              </Typography>
            </Grid>
  ...
  const handleSetEmployee = (employees: Select[]) => {
    ...
    setError({ ...error, selectedEmployee: '' });
  ...
  const renderAssign = () => {
    return (
      <Card variant='outlined' className={classes.card}>
        <CardHeader
          id='assign'
          title={<Typography variant='subtitle2'>Assign Technician & Vehicle</Typography>}
          style={{ backgroundColor: grey[200], height: 35 }}
        />
        <CardContent>
  ...
  const renderSettingSchedule = () => {
    return (
      <Card variant='outlined' className={classes.card}>
        <CardHeader
          id='settingSchedule'
          title={<Typography variant='subtitle2'>Schedule</Typography>}
          style={{ backgroundColor: grey[200], height: 35 }}
        />
        <ScheduleTabPanel
