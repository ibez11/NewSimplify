...
const BodyRow: FC<Props> = props => {
  ...
  const { isLoadingData, job, setSelectedId, setMessage, setOpenDialog, onViewJob, onEditJob, handleSnackbar, columns } = props;
  const {
    jobId,
    serviceId,
    serviceName,
          ...
          if (value.field === 'id') {
            ...
            return (
              <BodyCell cellWidth='20%' pR='5px'>
                {isLoadingData ? <Skeleton width={'80%'} /> : <Typography variant='body2'>{serviceAddress}</Typography>}
              </BodyCell>
            );
          } else if (value.field === 'jobStatus') {
            return (
              <BodyCell cellWidth='5%' pR='5px'>
                {isLoadingData ? <Skeleton width={'80%'} /> : <Typography variant='body2'>{ucWords(jobStatus.replace('_', ' '))}</Typography>}
              </BodyCell>
            );
                ...
                {isLoadingData ? (
                  ...
                  <div className={classes.tableCellInnerOther}>
                    <div className={classes.otherTextCell}>
                      <Chip
                        label={ucWords(serviceType === 'CONTRACT' ? 'SERVICE CONTRACT' : 'AD-HOC SERVICE')}
