...
const LOG = new Logger('AdminMobileJobExpensesController.ts');

@Controller('api/mobile/admin/job-expenses')
@ClassMiddleware(Authentication.AUTHENTICATED)
@ClassErrorMiddleware(globalErrorHandler)
export class AdminMobileJobExpensesController {
  ...
  private async add(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      await AppLogService.createAppLog(id, `[Admin Mobile] Create Job Expenses : ${jobId} - ${header}`);
  ...
  private async update(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      await AppLogService.createAppLog(id, `[Admin Mobile] Edit Job Expenses : ${editedJobExpenses.jobId} - ${header}`);
      return res.status(OK).json(row);
    } catch (err) {
      LOG.error(err);
      return next(err);
  ...
  private async delete(req: Request, res: Response, next: NextFunction) {
    ...
    try {
      ...
      await AppLogService.createAppLog(id, `[Admin Mobile] Delete Job Expenses : ${jobId} - ${header}`);
