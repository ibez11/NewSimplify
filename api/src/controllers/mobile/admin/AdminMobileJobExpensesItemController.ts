...
const LOG = new Logger('AdminMobileJobExpensesItemController.ts');

@Controller('api/mobile/admin/job-expenses-item')
@ClassMiddleware(Authentication.AUTHENTICATED)
@ClassErrorMiddleware(globalErrorHandler)
export class AdminMobileJobExpensesItemController {
  // for add job expenses item
  @Post('')
  ...
  private async add(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      await AppLogService.createAppLog(id, `[Admin Mobile] Create Job Expenses Item: ${jobExpensesId} - ${itemName}`);
  ...
  // for edit job expenses item based on job expenses item id
  ...
  private async update(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      await AppLogService.createAppLog(id, `[Admin Mobile] Edit Job Expenses : ${editedJobExpensesItem.id} - ${itemName}`);
  ...
  // for delete job expenses item based on job expenses item id
  ...
  private async delete(req: Request, res: Response, next: NextFunction) {
    ...
    try {
      ...
      await AppLogService.createAppLog(id, `[Admin Mobile] Delete Job Expenses : ${jobExpensesItemId} - ${itemName}`);
