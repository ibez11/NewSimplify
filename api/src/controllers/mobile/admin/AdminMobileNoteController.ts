...
const LOG = new Logger('AdminMobileNoteController.ts');

@Controller('api/mobile/admin/notes')
@ClassMiddleware(Authentication.AUTHENTICATED)
@ClassErrorMiddleware(globalErrorHandler)
export class AdminMobileNoteController {
  ...
  private async add(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      await AppLogService.createAppLog(id, `[Admin Mobile] Create Job Note : ${newJobNote.jobId} - ${newJobNote.notes}`);
  ...
  private async update(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      await AppLogService.createAppLog(id, `[Admin Mobile] Edit Job Note : ${editedJobNote.jobId} - ${editedJobNote.notes}`);
      return res.status(OK).json(row);
    } catch (err) {
      LOG.error(err);
      return next(err);
  ...
  private async updateVisibilityJobNote(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      await AppLogService.createAppLog(id, `[Admin Mobile] Edit Job Note : ${id} - ${isHide ? 'Hide' : 'Show'}`);
