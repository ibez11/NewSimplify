...
const LOG = new Logger('MobileJobController.ts');

@Controller('api/mobile/jobs')
...
export class MobileJobController {
  ...
  //for get all jobs with detail by parameter
  @Get('list')
  @Auth({ module: Modules.JOBS, accessLevel: AccessLevels.VIEW })
  private async getAllWithDetail(req: Request<void, void, void, JobQueryParams>, res: Response, next: NextFunction) {
    try {
      const { id } = req.user as JwtPayload;
      ...
      if (rows) {
        await Promise.all(
          rows.map(async row => {
            const result = await JobAttributes.toResponseMobile(row);
            jobs.push({ ...result, doneJob: row.doneJob ? Number(row.doneJob) : 0 });
          })
        );
      }
  ...
  // for update job status based on job id
  @Put(':jobId')
  @Auth({ module: Modules.JOBS, accessLevel: AccessLevels.EDIT })
  private async updateJobStatus(req: Request, res: Response, next: NextFunction) {
    try {
      const { id } = req.user as JwtPayload;
      ...
      await AppLogService.createAppLog(id, `[Mobile] Edit Job Status: ${jobId} - ${row.jobStatus}`);
      return res.status(OK).json({ newJobStatus: row.jobStatus });
    } catch (err) {
      LOG.error(err);
      return next(err);
    }
  ...
  private async updateJobServiceItems(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      await AppLogService.createAppLog(id, `[Mobile] Edit Job Service Item : ${jobId}`);
      return res.status(OK).json(result);
    } catch (err) {
      LOG.error(err);
      return next(err);
